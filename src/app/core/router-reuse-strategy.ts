...
// copied from https://github.com/angular/angular/issues/13869#issuecomment-441054267
...
function getConfiguredUrl(route: ActivatedRouteSnapshot): string {
  return '/' + route.pathFromRoot
    ...
    .map(v => v.routeConfig ? v.routeConfig.path : v.routeConfig)
    .join('/');
}

@Injectable()
export class RouterReuseStrategy implements RouteReuseStrategy {
  MAX_DEFAULT_ROUTES = 5;
      ...
      if (config) {
        ...
        if (handle) {
          if (!config.handles[storeKey]) {
            // add new handle
            if (config.handleKeys.length >= this.MAX_DEFAULT_ROUTES) {
